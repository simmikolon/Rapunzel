...
class ProgressScene: Scene {
    ...
    */
    ...
    fileprivate var downloadFailedObserver: AnyObject?
    ...
    /**
        ...
        assets into memory.
    */
    static func progressSceneWithSceneLoader(_ sceneLoader: SceneLoader) -> ProgressScene {
    ...
    func setupWithSceneLoader(_ sceneLoader: SceneLoader) {
        ...
        downloadFailedObserver = defaultCenter.addObserver(forName: NSNotification.Name(rawValue: SceneLoaderDidFailNotification), object: sceneLoader, queue: OperationQueue.main) { [unowned self] notification in
            guard let sceneLoader = notification.object as? SceneLoader, let error = sceneLoader.error else { fatalError("The scene loader has no error to show.") }
    ...
    deinit {
        ...
        if let downloadFailedObserver = downloadFailedObserver {
            NotificationCenter.default.removeObserver(downloadFailedObserver, name: NSNotification.Name(rawValue: SceneLoaderDidFailNotification), object: sceneLoader)
        }
    ...
    */
    ...
    func showErrorStateForError(_ error: NSError) {
